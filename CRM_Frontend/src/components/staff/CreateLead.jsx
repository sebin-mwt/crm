...
function CreateLead() {
  ...
  async function fetchMetaData() {
    try {
      ...
      const stageRes = await fetch(`${BASE_URL}/manager/lead/stage`, { headers: { Authorization: `Bearer ${token}` } })
      setStages(await stageRes.json())

      const statusRes = await fetch(`${BASE_URL}/manager/lead/status`, { headers: { Authorization: `Bearer ${token}` } })
      setStatuses(await statusRes.json())

      const serviceRes = await fetch(`${BASE_URL}/services`, { headers: { Authorization: `Bearer ${token}` } })
      setServices(await serviceRes.json())

      const customerRes = await fetch(`${BASE_URL}/customers`, { headers: { Authorization: `Bearer ${token}` } })
